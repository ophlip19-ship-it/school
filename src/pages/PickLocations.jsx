...
function plainCoords(point) {
  if (!hasLngLat(point)) return null;
...
export default function PickLocations() {
  ...
  const school =
    ...
    "";
  const rawSchoolCoords =
    plainCoords(childSchool.schoolCoords) ||
    plainCoords(draft.schoolCoords) ||
    schoolCoordsFromChild(selectedChild);
  const schoolLng = rawSchoolCoords?.lng ?? null;
  const schoolLat = rawSchoolCoords?.lat ?? null;
  const schoolCoords = useMemo(
    () => plainCoords({ lng: schoolLng, lat: schoolLat }),
    [schoolLng, schoolLat],
  );
  const homeLng = user?.homeCoords?.lng ?? null;
  const homeLat = user?.homeCoords?.lat ?? null;
  ...
  const initialModes =
    draft.pickupMode || draft.dropoffMode
      ? {
  ...
  const placeMarker = useCallback((kind, place) => {
    if (!map.current || !place || place.lng == null || place.lat == null)
      return;
    ...
    if (ref.current) {
      ...
      ref.current = new mapboxgl.Marker({ color })
        ...
        .setPopup(
          new mapboxgl.Popup({ offset: 18 }).setText(place.label || kind),
        )
        .addTo(map.current);
    }
  }, []);
  ...
  const placeArgs = {
    child: selectedChild,
    homeAddress,
    homeCoords: plainCoords(user?.homeCoords),
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setPickupPlace((prev) => (samePlace(prev, from) ? prev : from));
        if (pickupMode === "current") {
          setCustomPickup((prev) => (prev === from.label ? prev : from.label));
        }
      } catch (err) {
        if (!cancelled) {
          setError(
    ...
    // placeArgs fields listed as primitives so object identity cannot retrigger
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setDropoffPlace((prev) => (samePlace(prev, to) ? prev : to));
      } catch (err) {
        if (!cancelled) {
  ...
  useEffect(() => {
    if (!mapReady || !map.current || !hasLngLat(pickupPlace) || !hasLngLat(dropoffPlace)) {
      setFareQuote((q) => (q == null ? q : null));
      return undefined;
    }
    let cancelled = false;
    (async () => {
    ...
    // Depend on coordinate primitives so a new place object with the same pin
    // does not refetch the route.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    mapReady,
    pickupPlace?.lng,
    ...
    try {
      ...
      if (!hasLngLat(from)) {
        from = await resolvePlace({
          ...
          customCoords: plainCoords(pickupPlace),
        });
      }
      if (!hasLngLat(to)) {
        to = await resolvePlace({
          ...
          ...placeArgs,
          customLabel:
            customDropoff || dropoffPlace?.label || school || "School",
          customCoords: plainCoords(dropoffPlace),
        });
      }

      const pickupCoords = plainCoords(from);
      const dropoffCoords = plainCoords(to);
      if (!pickupCoords || !dropoffCoords) {
        throw new Error(
          "Choose a pickup and destination on the map before continuing.",
        );
      }
      ...
      setBookingDraft({
        ...
        schoolAddress: school,
        schoolCoords: plainCoords(schoolCoords),
      ...
      // Do not await GPS here — geolocation prompts can hang the tab.
      captureParentLocationForBooking().then((parentLocation) => {
        if (parentLocation) setBookingDraft({ parentLocation });
      });
    } catch (err) {
      setError(
        err?.message?.includes("denied") || err?.code === 1
      ...
      {mapError && (
        <div className="absolute inset-x-4 top-24 z-30 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900 shadow">
          ...
          <p className="mt-2 text-xs">
            You can still use the form below once the drawer is open, or set
            VITE_MAPBOX_TOKEN.
          </p>
        </div>
      )}
        ...
        <div>
          <label className="mb-2 flex items-center justify-between gap-2 text-sm font-semibold text-slate-800">
            <span className="inline-flex items-center gap-2">
              <MapPin size={16} className="text-emerald-600" /> Pickup for
              driver
            </span>
